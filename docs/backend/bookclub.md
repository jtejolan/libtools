...
| `BookClubMember` | `bookclub_members` | A club's member roster; `email` is optional (nullable) and unique per `(club_id, email)` only when set — a member without an email can't self-register/link a participant account by email match, and onboarding/arrival/reminder emails skip or reject members with no address on file; carries delivery details, transit-label/email timestamps, and an optional participant-controlled directory profile (`bio`, `avatar_url`, `directory_visible`) |
